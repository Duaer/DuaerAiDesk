...
| D007 | `~/.pi` compatibility | **No auto-import in MVP** | Keep config ownership clean in `~/.duaer-ai-desk` |
...
| D457 | Signed macOS DMG is a two-icon install | **Amend D406 / ADR 0232 / ADR 0204: official and local DMGs contain only DuaerAiDesk.app and the Applications link on a branded 720×440 plate. They no longer include `If app won't open, read this.txt` or the command helper. macOS ZIP packages still ship both the opening note and `DuaerAiDesk-macOS-open.command` for trusted unsigned artifacts. See ADR 0296 and E2E-196b.** | Tagged DMGs are signed and notarized (D450); the unsigned-opening note on the installer read as an error. |
...
| D032 | Workspace ignore | **security denylist + defaults + `.duaer-ai-deskignore`** | Safe/predictable tool FS behavior |
...
| ID | Topic | Decision | Rationale |
|---|---|---|---|
| D034 | Desktop visual baseline | **Codex electron-dark 1:1 shell (charcoal gray, floating composer, ~275px sidebar)** | Match local Codex usability and density; keep DuaerAiDesk product branding |
| D035 | Shell display name | *(superseded by D094)* **UI chrome uses shellName "Codex"; product/about remains DuaerAiDesk** | Satisfy visual 1:1 replica goal while preserving product identity in about/settings |
...
| D057 | Home mark + hero title optical | *(home mark superseded by D094; title guidance retained)* **Empty-home Codex mark uses denser stroke; short workspace basenames display as `DuaerAiDesk` for gold title span** | Hero residual was thin mark + short project label under-inking title vs Codex gold |
...
| D084 | Cross-platform shell strategy | *(superseded by D190)* **The Bash tool runs bash on every platform, resolved once per process: `DUAER_AI_DESK_BASH` override → Unix well-known paths + PATH → Windows `bash.exe` derived from Git for Windows (git on PATH, standard install dirs, then PATH minus the WSL `System32` launcher); Unix uses `bash -lc`, Windows `bash -c` + `CREATE_NO_WINDOW`; no bash bundled in installers; missing shell surfaces stable `SHELL_NOT_FOUND` with install guidance** | Superseded by the selectable shell catalog and stable execution identity in D190 |
...
| D354 | Label both macOS release architectures | **Amend D353 / ADR 0145: both native macOS release lanes pass target-specific electron-builder patterns. Public assets are `DuaerAiDesk-<version>-arm64.dmg` / `DuaerAiDesk-<version>-arm64-mac.zip` and `DuaerAiDesk-<version>-x64.dmg` / `DuaerAiDesk-<version>-x64-mac.zip`. Generated per-architecture updater feeds retain these URLs and checksums. This changes release asset naming only; updater ownership, signing, and delivery mode remain unchanged.** | The arm64 asset `DuaerAiDesk-0.14.4.dmg` did not identify its architecture, while the x64 lane used a different `Intel` convention. Both public architectures need self-describing, standard labels. |
| D353 | Disambiguate Intel macOS release artifacts | **Amend D285 / ADR 0145: the native macOS x64 release lane passes target-specific electron-builder patterns so its public assets are `DuaerAiDesk-<version>-Intel.dmg` and `DuaerAiDesk-<version>-Intel-mac.zip`. The generated `latest-mac-x64.yml` retains those URLs and checksums. The arm64 lane keeps the generic `DuaerAiDesk-<version>.dmg` and `DuaerAiDesk-<version>-mac.zip` names. This changes release asset naming only; updater ownership, signing, and delivery mode remain unchanged.** | Users could not reliably distinguish the Intel download from the arm64 download when both used generic version names. Naming the x64 assets at build time also prevents the updater feed from retaining stale pre-rename URLs. |
...
| D599 | A development build is its own installation | **Narrow D236 / amend ADR 0094: a development build (unpackaged, or `DUAER_AI_DESK_DEV=1`) takes `DuaerAiDesk Dev` as its Electron `userData` — and with it the single-instance lock, renderer `localStorage`, the plugin panel partitions, and browser pane cookies — and reads `~/.duaer-ai-desk-dev`. An explicit `--user-data-dir` still wins, which is how the E2E harnesses point a build at a throwaway profile. A packaged installation keeps `DuaerAiDesk` and `~/.duaer-ai-desk`, so no existing profile is relocated. `DUAER_AI_DESK_DATA_DIR` still overrides either profile and is made absolute before it reaches host-core as a child-process environment variable; Electron main publishes the resolved directory back to that variable so the plugin runtime reads one root. No IPC, protocol, schema, or packaged-installation path change. See `03-runtime/07-process-model.md` and E2E-150.** | A packaged app that was already running held the lock, so `pnpm dev` quit on arrival; a development host that won the race instead put a second host-core over the same single-writer `pi.sqlite`, the outbox, and the log tree. |
| D602 | Crash dumps stay in the data directory | **Electron's Crashpad reporter starts local-only (`uploadToServer: false`) before `ready`. Dumps live under `<data_dir>/crash-dumps`, not the default Electron `userData` crashDumps path, so a `DUAER_AI_DESK_DATA_DIR` profile does not share dumps. The next lock-holding launch writes one `diagnostics` line for dumps newer than `crash-dumps.json`, classified by Crashpad `ptype`: `error` if any new dump is the browser/main process, `warn` for recovered renderer/GPU/utility crashes. Host-core and sidecar crashes stay on the supervisor path. No upload, no IPC, no schema change.** | A crash left a minidump nobody read. Crashpad also records recovered renderer crashes, so a next-launch `error` that said the previous run died was a lie; and dumps outside the data directory escaped `DUAER_AI_DESK_DATA_DIR` isolation. |
...
| D313 | Main-owned GitHub issue feedback | **GitHub issue forms are the only intake path. The bug form requires description, reproduction steps, expected and actual behavior, app version, and OS; the feature form requires a problem and a proposed change. Settings → Info exposes one Report a problem action on `duaer-ai-desk/app/openFeedback`. Electron Main builds a fixed `bug_report.yml` URL, prefills `app-version` / `os` / `environment` from Main-owned version info, and opens it with `shell.openExternal`. The renderer cannot supply a URL. Feature requests stay on GitHub's template picker. No host-protocol, storage, or update-feed change (ADR 0157).** | Reports without version or steps cannot be triaged, and a renderer-chosen destination would weaken the same Main-owned GitHub URL rule as D120. |
...
| D121 | Branded macOS development host | **`pnpm dev` on macOS launches electron-vite through a fingerprinted, ad-hoc-signed DuaerAiDesk copy of the installed Electron host bundle under `.cache/electron-dev/`. That ad-hoc signature keeps hardened runtime and `apps/desktop/build/entitlements.mac.plist`, so a terminal-launched host can rasterize Apple Color Emoji. The generated bundle changes only development host metadata, executable name, bundle identifier, and the ICNS resource; it never mutates `node_modules`. Windows/Linux keep the stock development executable, while packaged lanes remain electron-builder-owned.** | AppKit ignores runtime app-name/menu overrides for the top-level application identity and takes the native menu name and About icon from the host bundle; a branded development host is required for parity with packaged DuaerAiDesk. Re-signing that copy without hardened runtime crashes the main process in ImageIO when the input-method candidate bar draws an emoji bitmap. |
| D129 | Menu-free Windows/Linux window chrome | **The application menu is a macOS system-menu surface only. Windows/Linux retain the shared frameless 46px titlebar and renderer-drawn minimize/maximize-or-restore/close controls, but render no File/Edit/View/Window/Help menu inside the window and reserve no left-side titlebar space for one. Existing application, editing, zoom, fullscreen, and close shortcuts remain available through renderer/native web-content handling; update checks remain reachable from Settings -> Info. This supersedes only the Windows/Linux renderer-menubar portions of D118 and ADR 0021.** | An in-window desktop menu duplicates macOS-specific system-menu chrome, consumes navigation space, and does not belong in DuaerAiDesk's frameless Windows/Linux titlebar. |
...
| D141 | Canonical Windows native application identity | *(amended by D443)* **Electron Main sets the product name before readiness and registers `com.duaer-ai-desk.app` as the Windows process AppUserModelID before creating any window. That ID is the existing electron-builder/NSIS application ID; Windows packaging explicitly retains `DuaerAiDesk` for the executable and Start menu shortcut. Native notification attribution, notification settings, taskbar grouping, installed shortcuts, and packaged executable identity must expose `DuaerAiDesk`, not the stock Electron host. D121 remains unchanged: Windows development may use the stock Electron executable while its OS-facing runtime identity uses the canonical AUMID.** | `app.setName` changes Electron's internal name but not the Windows identity used by notifications and shell integration. One stable ID across runtime and packaging prevents both the observed notification-source leak and adjacent shell-brand drift without changing the published NSIS upgrade identity. |
...
| D124 | `@` file references are plain-text light references | **`@` at a token boundary opens a fuzzy file menu over a workspace index served by the new Electron-only channel `duaer-ai-desk/fs/index` (files+dirs, `git ls-files -co --exclude-standard` fast path with ignore-set walk fallback, 8000-entry cap with truncation flag, short TTL cache, workspace-rooted, fails soft without a workspace). Accepting creates the canonical `@rel/path ` reference for files (quoted `@"a b.txt" ` when the path has spaces) and inserts `@dir/` without trailing space for directories; D209 amends completed-file draft presentation without changing the dispatched plain-text path. The model follows references with its Read tool in both Plan and Agent. No prompt content inlining or provider attachment conversion; clipboard file materialization is defined by D197.** | Matches pi CLI's model-facing semantics, avoids context inflation and truncation rules, and keeps user-driven file browsing out of the agent tool/permission path per ADR 0019. |
...
| D371 | Explicit unsigned macOS first-launch helper | *(amended by D406 and D443)* **Every macOS distribution ships an executable `DuaerAiDesk-macOS-open.command`, placed on the DMG in a visible first-launch row below the drag-to-Applications gesture. It searches only `/Applications/DuaerAiDesk.app` and `~/Applications/DuaerAiDesk.app`, verifies `CFBundleIdentifier` is `net.aiuo.duaer-ai-desk`, removes only `com.apple.quarantine` when present, and opens the app. It never uses `sudo`, accepts no arbitrary path, and does not replace Developer ID signing or notarization.** | The unsigned macOS lane can be blocked by quarantine with a misleading damaged-app message, and the terminal-only `xattr -cr` note was broader than the launch failure requires (ADR 0204, E2E-196b) |
| D443 | Canonical application ID and macOS codesign identifier | **Amend D141 / D371 / ADR 0204: the application ID is `net.aiuo.duaer-ai-desk` (`APP_ID`, electron-builder `appId`, macOS `CFBundleIdentifier`, Windows AppUserModelID). Development macOS hosts use `net.aiuo.duaer-ai-desk.dev`. Do not adhoc-sign the unsigned pack in `afterPack`/`afterSign`: nested Electron helpers are still unsigned and `codesign` fails with `code object is not signed at all`. See ADR 0278 and issue #524.** | The owner domain is `net.aiuo.duaer-ai-desk`. Binding the unsigned outer identifier is deferred until a helper-safe pack path exists. |
...
- Dedicated repo `vastsa/duaer-ai-desk-plugins` is the official marketplace source.
- DuaerAiDesk fetches `catalog.json` remotely (cached under `~/.duaer-ai-desk/plugins/market/`).
...
- DuaerAiDesk marketplace UI now opens a detail pane with README, changelog, and version list via `market.getDetail`.
...
- DuaerAiDesk now evaluates context after every `turn_end`, before the next
...
- Plugin authoring ships as a first-party package, `@duaer-ai-desk/plugin-devkit`,
...
- Both Codex families exist: the summary path, and a `fresh_window` rollover
  ...
  `DUAER_AI_DESK_COMPACTION_STRATEGY`), absent from settings and i18n, because no
...
## 2026-08-14 — DuaerAiDesk stays a regular macOS application
...
- The plugin launcher window asked Electron for `visibleOnFullScreen` without
  ...
  process and DuaerAiDesk vanished from the Dock and Cmd+Tab. ADR 0080's boot
...
- The launcher keeps `canJoinAllSpaces` and `FullScreenAuxiliary`: it still
  covers every regular Space and DuaerAiDesk's own fullscreen window. Overlaying
  another application's fullscreen Space is given up — macOS reserves it for
  accessory apps — and invoking the launcher from one activates DuaerAiDesk.
...
- Launching DuaerAiDesk while it is already running no longer starts a second
...
- Decision D236 protects D002's single SQLite writer and the singletons around
  ...
  installation, so runs with their own `DUAER_AI_DESK_DATA_DIR` — E2E harnesses,
...
- Electron main owns login/logout: `oauth.ts` implements pi-ai's
  ...
  `duaer-ai-desk/providers/oauth/event` stream. Browser-callback, device-code,
...
- Bundled plugins carry `source: "builtin"`. host-core reconciles its registry
  against the directory Electron reports in `DUAER_AI_DESK_BUILTIN_PLUGINS_DIR` on
...
- Decision D252 amends D216 / ADR 0078 for the native Windows taskbar path.
  ...
  minimize and keeps the DuaerAiDesk taskbar entry. Clicking it again restores
...
- The composer has one submit slot. While a run is active, a non-empty draft
  ...
  `duaer-ai-desk/agent/stop`; the sidecar sets pi-agent-core's one-shot
...
- Rows stay `openai_compatible` + `chat_completions`. pi-ai already speaks
  those hosts; DuaerAiDesk does not add a Zhipu apiStyle or SDK.
...
- UI locales now live in `@duaer-ai-desk/i18n`: `en`, `zh-CN`, and `tr`. Native
...
- Codex searches with shell `rg` when the machine has it. DuaerAiDesk keeps
...
- The result shape, 128KB / `headLimit` budget, newest-first order, and
  ...
  to the in-process `ignore` + `regex` searcher. `DUAER_AI_DESK_RG` selects a
  binary; `DUAER_AI_DESK_DISABLE_RG` forces the fallback.
...
- The transcript now renders those tokens as composer-matching leaf-name
  ...
  with the OS default application via `duaer-ai-desk/fs/open`.
...
- Decision D337 amends ADR 0116: agent-runtime sends `x-opencode-session`,
  `x-opencode-client: duaer-ai-desk`, and a DuaerAiDesk `User-Agent` on session,
...
- Decision D353 amends D285 / ADR 0145: the Intel x64 lane passes target-specific
  electron-builder patterns and publishes `DuaerAiDesk-<version>-Intel.dmg` and
  `DuaerAiDesk-<version>-Intel-mac.zip`. The x64 updater feed keeps those URLs
...
- D353 left the arm64 macOS asset with a generic version-only name while using
  `-Intel` only for the x64 asset, so `DuaerAiDesk-0.14.4.dmg` still did not
...
- Windows tag releases published only `DuaerAiDesk-Setup-<version>.exe`, which
...
- Decision D364 amends D120 / D126 / ADR 0022: the Windows x64 lane publishes
  `DuaerAiDesk-Portable-<version>.exe` alongside the NSIS installer. The
...
- Decision D603 amends D364 / ADR 0022 / ADR 0197. The Windows x64 lane now
  publishes `DuaerAiDesk-Portable-<version>.zip` beside the NSIS installer.
  ...
  `latest.yml`.
- Users extract the ZIP and launch `DuaerAiDesk.exe` directly. The ZIP run uses
...
- Electron main emits `duaer-ai-desk/session/event/changed` after successful plugin
...
- Decision D371 / ADR 0204 adds an executable `DuaerAiDesk-macOS-open.command`
  to every macOS distribution. The DMG places it in a visible first-launch row
  below the drag-to-Applications gesture.
- The helper searches only `/Applications/DuaerAiDesk.app` and
  `~/Applications/DuaerAiDesk.app`, verifies `CFBundleIdentifier` is
  `com.duaer-ai-desk.app`, removes only `com.apple.quarantine` recursively when
  present, and opens DuaerAiDesk. It never uses `sudo`, accepts no arbitrary
...
- Named tools cover the common project/session/Agent flow; `duaer_ai_desk_invoke`
...
- Decision D375 amends ADR 0205 a second time: rollout R2 becomes the
  SSH-tunnel remote Host, with a `duaer-ai-desk-host` bundle downloaded from GitHub
...
- Decision D385 amends ADR 0205 a third time and withdraws D375 item 10: no
  ...
  download of `duaer-ai-desk-host`. The SSH-tunnel topology, device pairing, and the
...
- In the same change, without new decisions: `pi.net.fetch` has one path,
  ...
  (`WORKSPACE_PATH_DENIED`), app defaults, `.duaer-ai-deskignore`, and
...
- The timing-only process log streams introduced by D137/D183 are retired:
  ...
  `DUAER_AI_DESK_TIMING` are no longer emitted.
...
- D387 shipped ExtensionAPI modules as a second surface with its own
  ...
  surface: a pi CLI extension should become a DuaerAiDesk plugin and be
...
- pi-ai auto-detects that flag only when `model.provider === "deepseek"` or the
  base URL contains `deepseek.com`. DuaerAiDesk stores a UUID as `model.provider`,
...
- macOS DMGs expose `DuaerAiDesk-macOS-opening-help.txt` with the Finder label
...
- The desktop installs and pairs a `duaer-ai-desk-host` on a machine the user already
...
- A paired host now stores an SSH descriptor (`metadata.transport = "ssh"`)
  ...
  arguments.
- `duaer-ai-desk/remoteHost/bootstrap` joins `list` / `pair` / `remove`, and the
...
- A packaged DuaerAiDesk and a `pnpm dev` host no longer share the two
  directories that define an installation. A development build runs under
  `DuaerAiDesk Dev` in the OS application-data root — its own Electron
  ...
  `~/.duaer-ai-desk-dev`. A packaged installation keeps `DuaerAiDesk` and
  `~/.duaer-ai-desk` unchanged, so no existing profile is relocated.
- A shipped app that is already running therefore no longer refuses the lock a
  ...
  `pi.sqlite`, one persistence outbox, or one log tree. `DUAER_AI_DESK_DATA_DIR`
...
- The Settings Font picker keeps its shape: System default persists the empty
  ...
  `duaer-ai-desk/app/systemFonts` using platform tooling only (deduplicated,
...
- Crashpad starts local-only (`uploadToServer: false`) before `ready`. Dumps
  ...
  crashDumps path, so a `DUAER_AI_DESK_DATA_DIR` profile does not share dumps
...
- The renderer gains the quit channel `duaer-ai-desk/app/quit` (`api.quitApp()`),
...
- A checkpoint's `readFiles` / `modifiedFiles`, and the `<read-files>` section a
  ...
  DuaerAiDesk registers `Read` / `Write` / `Edit`, so the collector matched
