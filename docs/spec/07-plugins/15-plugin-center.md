...
> Distribution repository: [vastsa/duaer-ai-desk-plugins](https://github.com/vastsa/duaer-ai-desk-plugins)
...
| `.piplug` artifact | Center (published) | `vastsa/duaer-ai-desk-plugins/packages/` |
| Catalog | Center (generated) | `vastsa/duaer-ai-desk-plugins/catalog.json` |
| Distribution mirror | Center | CNB Git mirror of the distribution repository |
| Install decision | User | Desktop permission review |

Plugin source is never copied into a DuaerAiDesk-owned Git repository. The center
...
vastsa/duaer-ai-desk-plugins               # distribution, unchanged addresses
...
3. Write the registry entry in `pi-plugin-center` and regenerate the catalog from
   it.
4. Commit the package and the regenerated `catalog.json` to `duaer-ai-desk-plugins`
   in **one** commit.
...
- Client: catalog v2 parsing, download host allowlist, yank enforcement,
  `minDuaerAiDesk` enforcement, provenance capture and display, `pi-plugin publish`.
...
- Copying publisher source into a DuaerAiDesk repository.
