...
DuaerAiDesk already has typed Electron-main IPC for projects, sessions, the pi
...
- Add an optional Streamable HTTP MCP server inside Electron Main. It starts
  only when `DUAER_AI_DESK_MCP_CONTROL=1` is set and binds to `127.0.0.1`; an
  optional `DUAER_AI_DESK_MCP_PORT` selects the local port and defaults to 37123.
...
- Delegate every exposed call to the existing registered main-process IPC
  ...
  flow and a generic `duaer_ai_desk_invoke` over an explicit, risk-tagged catalog.
...
- After successful **mutating** project/session/Agent calls, reuse the existing
  `duaer-ai-desk/session/event/changed` renderer event with additive project and
