# ADR 0278: Canonical Application ID `net.aiuo.duaer-ai-desk`

- Status: Accepted
- Date: 2026-09-17
- Deciders: DuaerAiDesk packaging maintainers
...
The product advertised `com.duaer-ai-desk.app` as the application ID, but the
owner domain is `net.aiuo.duaer-ai-desk`. Default unsigned macOS packs also left
Electron's adhoc signature on `DuaerAiDesk.app` (`Identifier=Electron`,
...
1. The canonical application ID is **`net.aiuo.duaer-ai-desk`**. It is
   ...
   Development macOS hosts use `net.aiuo.duaer-ai-desk.dev`.
2. NSIS/AppUserModelID follow the same ID. Existing `com.duaer-ai-desk.app`
...
- Windows upgrades from a `com.duaer-ai-desk.app` NSIS install are a new
  product identity; users may see a parallel shortcut until the old install
  is removed.

## Alternatives considered

- Keep `com.duaer-ai-desk.app` and only re-sign: rejected because the owner
  domain is `net.aiuo.duaer-ai-desk`.
