...
- 左侧设置导航宽 **275px**，与主侧栏共享 `sidebar-surface` 材质：macOS 使用原生
  ...
  滚动容器、导航背景本身不淡入、不位移。设置弹框和抽屉挂到文档根上的 `#duaer-ai-desk-overlays`，遮罩覆盖整窗含导轨。
...
- 带有图标和简洁平行标签的紧凑导航目录，按以下顺序：
  ...
  8. **数字员工** — Lucide `Bot`（内置与自建的并行工作智能体）
  9. **导入** — Lucide `Download`（从其他工具引入会话和模型配置）
  10. **项目** — Lucide `Archive`（持久项目索引）
  11. **部署** — Lucide `KeyRound`（GitHub Pages 用本机已登录的 gh；Cloudflare、阿里云、AWS 的密钥只存在本机，保存后不再显示，不写入仓库）
  12. **云同步** — Lucide `CloudDownload`（加密的可移植配置备份与双向同步；仅开发者模式）
  13. **远程主机** — Lucide `Globe`（SSH 引导与配对清单；仅开发者模式）
  14. **信息** — Lucide `Info`（版本、日志、更新、开发人员）
...
- 目录仍是保持相同顺序的可搜索扁平列表。为便于扫描，目的地分为四个带标题
  ...
  MCP、数字员工）、“工作区”（导入、项目、部署）和“系统”（云同步、远程主机、信息；前两项仅开发者模式可见）。标题使用柔和
...
- **提供商**工作室：
  ...
  - 已登录的厂商行带账户徽标，其编辑对话框用该徽标替换 API 密钥字段 ——
    这里没有东西可粘贴
- 列表仍分两组：内置数字员工（`explorer`、`code-reviewer`、`test-runner`、
  `fixer`、`ui-designer`、`coder`、`judge`）和 `~/.agents/subagents` 下的用户文档。`judge`（Duaer 判断师）只有设了判断模型才会出现在 Task 里，编辑时也只能选这个模型。同名的已启用用户文档
  会在 `Task` 目录中遮蔽对应的内置定义，内置行随之省略、只保留用户行；同名的已停用用户
  文档会让该内置重新留在目录（以及内置列表）中，因为 `Task` 又用回随应用发布的定义。
  内置行带来源角标、编辑（只改模型，说明保持内置原样，保存在应用本地状态里）、
  「复制为我的定义」（以该定义预填新建表单，并选中对应的模板芯片），
...
- 子智能体新建/编辑表单使用与 Composer 相同的已配置、可运行模型，
  ...
  内置数字员工保留自己的内置行。编辑只打开模型选择器，模型保存在应用本地，下一次目录加载生效，关掉的行也一样。复制为我的定义仍然会新建用户文档。完整编辑表单只用于新建和用户自有数字员工。
  ...
  修复者、UI 设计师、Duaer 编码师、空白开始）：只显示名称，选中项的一句话说明出现在整行下方。带连字符的
...
### 说明（`instructions` 选项卡）
- 编辑每个 DuaerAiDesk Agent 会话使用的全局指令 Markdown。
...
- **问题反馈**行：一项操作在系统浏览器中打开 GitHub bug 表单。
  Electron Main 拥有该 URL（`duaer-ai-desk/app/openFeedback`），用主进程版本信息
