...
SSH 引导配对（首个远程拓扑）：桌面经用户自己的 SSH 会话在远端启动 `duaer-ai-desk-host`，
...
Releases 下载与桌面同版本、对应远端平台的 `duaer-ai-desk-host` 包，校验随发布公布的 SHA-256 后
...
并经 SSH 端口转发访问的 `duaer-ai-desk-host`，在绑定地址与对端地址都是 loopback 且出示
...
Agent 的参数不传 secret，中继连接丢失则工具失败而回合继续。会话终端是以 `duaer-ai-desk-host`
...
（Gateway 里程碑排期后适用）、绑定 loopback 的 `duaer-ai-desk-host` 只接受出示有效设备
...
D375（2026-09-10）新增 SSH 引导配对、经 SSH 端口转发的 `duaer-ai-desk-host` loopback 规则、
SSH 配对 owner 设备的上限豁免、远程工具目录与 provider 配置规则、验收门 16–18，
并把 Gateway 与 cookie profile 条款标记为属于不排期的里程碑。

D375 同日记录的设计决定把身份源定为 PI 账号服务，`duaer-ai-desk-host` 从 GitHub Releases 下载，
