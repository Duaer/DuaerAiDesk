#!/usr/bin/env python3
"""Derive DuaerAiDesk platform icon resources from the canonical logo.
...
def main() -> None:
    ...
    # dark DI mark as a transparent, monochrome image instead.
    ...
    # tile. Keep the DI letter area so that outline cannot leak into the
    # menu bar template silhouette.
    mark_clip = Image.new("L", master.size, 0)
    mark_draw = ImageDraw.Draw(mark_clip)
    mark_draw.rectangle((180, 240, 860, 820), fill=255)
    ...
    if mark_bounds is None:
        raise ValueError("canonical logo does not contain a dark DI mark")
