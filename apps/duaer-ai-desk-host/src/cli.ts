...
/**
 * `duaer-ai-desk-host [--data-dir <dir>] [--port <n>] [--pair] [--host-core <bin>] [--sidecar <entry>]`
 *
...
async function main(): Promise<void> {
  ...
  if (args.help === true) {
    process.stdout.write(
      [
        "duaer-ai-desk-host — headless PI Agent Host (RACP-WS on loopback)",
        "",
        "  --data-dir <dir>          host data directory (default ~/.duaer-ai-desk)",
        ...
        "  --host-core <path>        duaer-ai-desk-host-core binary",
  ...
  process.stdout.write(`PI_HOST_READY ${JSON.stringify({ hostId: app.hostId, host: app.address.host, port: app.address.port, version: (await import("@duaer-ai-desk/shared")).APP_VERSION })}\n`);
  if (config.pair) {
...
main().catch((error) => {
  ...
  process.stderr.write(`${JSON.stringify({ ts: new Date().toISOString(), level: "error", channel: "duaer-ai-desk-host", message: "duaer-ai-desk-host failed to start", data: { code, error: error instanceof Error ? error.message : String(error) } })}\n`);
