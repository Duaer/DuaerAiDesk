...
export type PiHostConfig = {
  dataDir: string;
  /** Loopback bind address; `duaer-ai-desk-host` refuses anything else. */
  host: string;
...
export function hostCoreCandidates(root = here()): string[] {
  ...
  return [
    ...
    process.env.DUAER_AI_DESK_HOST_BIN ?? "",
    join(root, `bin/duaer-ai-desk-host-core${exe}`),
    join(root, `../bin/duaer-ai-desk-host-core${exe}`),
...
export function resolveConfig(args: CliArgs, env: NodeJS.ProcessEnv = process.env): PiHostConfig {
  const dataDir = resolve(String(args["data-dir"] ?? env.DUAER_AI_DESK_DATA_DIR ?? join(homedir(), ".duaer-ai-desk")));
