...
import { AgentHost, type ApprovalPort } from "@duaer-ai-desk/agent-host";
import {
  AgentSidecar,
  HostProcess,
...
import { DeviceTokenAuthenticator, RacpServer, bindRacpWebSocket, type RacpHostOperations, type WsBinding } from "@duaer-ai-desk/racp";
import { APP_VERSION, type AgentEventEnvelope } from "@duaer-ai-desk/shared";
...
export async function startPiHost(config: PiHostConfig, options: { log?: HostLogger } = {}): Promise<PiHostApp> {
  ...
  const startSidecar = async () => {
    const sidecar = new AgentSidecar({
      launch: { command: config.nodeBinary, args: [config.sidecarEntry], env: { ...process.env, DUAER_AI_DESK_DATA_DIR: config.dataDir } },
    ...
    sidecar.setProjectInstructionResolver(async ({ projectPath, path }) => {
      const { loadInstructionChain } = await import("@duaer-ai-desk/agent-runtime");
  ...
  log("info", "duaer-ai-desk-host ready", { hostId, host: binding.address.host, port: binding.address.port, version: APP_VERSION, terminal: Boolean(terminal) });
  ...
  return {
    ...
    async stop() {
      ...
      state.stopping = true;
      log("info", "duaer-ai-desk-host stopping");
      server.close();
      await binding.close();
