...
async function tempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), "duaer-ai-desk-host-test-"));
  dirs.push(dir);
  return dir;
}
...
describe("identity and credentials", () => {
  it("mints the host id once and keeps it across restarts", async () => {
    ...
    const info = await stat(join(dir, "duaer-ai-desk-host", "identity.json"));
  ...
  it("stores devices and pairings hashed, owner-readable, and survives a reload", async () => {
    ...
    const raw = await readFile(join(dir, "duaer-ai-desk-host", "credentials.json"), "utf8");
    expect(raw).not.toContain("pdt1.");
    expect(raw).toContain("ab".repeat(32));
    expect(raw).not.toContain("ef".repeat(32));
    const info = await stat(join(dir, "duaer-ai-desk-host", "credentials.json"));
    expect(info.mode & 0o077).toBe(0);
