...
import type { DeviceCredentialStore, DeviceRecord, PairingRecord } from "@duaer-ai-desk/racp";
...
export async function loadOrCreateHostId(dataDir: string): Promise<string> {
  const path = join(dataDir, "duaer-ai-desk-host", "identity.json");
...
/**
 * Device and pairing records under `<dataDir>/duaer-ai-desk-host/credentials.json`,
...
export class FileCredentialStore implements DeviceCredentialStore {
  ...
  constructor(dataDir: string) {
    this.path = join(dataDir, "duaer-ai-desk-host", "credentials.json");
  }
