...
/**
 * Structured NDJSON on stderr, mirrored to `<dataDir>/logs/duaer-ai-desk-host.log`.
...
export function createLogger(options: { dataDir: string; minLevel: LogLevel; stderr?: NodeJS.WritableStream }): HostLogger {
  ...
  const write = (record: Record<string, unknown>) => {
    ...
    stderr.write(line);
    void ready.then(() => appendFile(join(logDir, "duaer-ai-desk-host.log"), line).catch(() => undefined));
  };
  const log = ((level, message, data) => {
    if (LEVEL_RANK[level] < LEVEL_RANK[options.minLevel]) return;
    write({ ts: new Date().toISOString(), level, channel: "duaer-ai-desk-host", message, ...(data ? { data: redactLogData(data) } : {}) });
  }) as HostLogger;
