#!/usr/bin/env node
/**
 * Assemble the self-contained `duaer-ai-desk-host` bundle for one Linux target:
 *
 *   dist-bundle/duaer-ai-desk-host-<version>-<platform>-<arch>/
 *     duaer-ai-desk-host.js                 the CLI, esbuild-bundled with every workspace package
 *     agent-runtime/sidecar.js   the same sidecar bundle the desktop ships
 *     bin/duaer-ai-desk-host-core   the platform host-core binary
 *     node_modules/node-pty      optional; terminals are disabled without it
 *     package.json               { type: module, version }
 *     install.sh                 copies the bundle under ~/.duaer-ai-desk/duaer-ai-desk-host/<version>
 *
...
const hostCore = resolve(String(args["host-core"] ?? join(root, `target/release/duaer-ai-desk-host-core${exe}`)));
const sidecar = join(root, "packages/agent-runtime/dist-bundle/sidecar.js");
const out = resolve(String(args.out ?? join(app, "dist-bundle", `duaer-ai-desk-host-${version}-${platform}-${arch}`)));
...
execFileSync(
  ...
  [
    ...
    `--outfile=${join(out, "duaer-ai-desk-host.js")}`,
...
cpSync(hostCore, join(out, `bin/duaer-ai-desk-host-core${exe}`));
chmodSync(join(out, `bin/duaer-ai-desk-host-core${exe}`), 0o755);
try {
...
writeFileSync(join(out, "package.json"), `${JSON.stringify({ name: "duaer-ai-desk-host", version, type: "module", bin: { "duaer-ai-desk-host": "./duaer-ai-desk-host.js" } }, null, 2)}\n`);
writeFileSync(
  join(out, "install.sh"),
  `#!/bin/sh
# Install this duaer-ai-desk-host bundle under the user's home (D375 bootstrap).
set -eu
target="\${PI_HOST_INSTALL_DIR:-$HOME/.duaer-ai-desk/duaer-ai-desk-host}/${version}"
mkdir -p "$target"
cp -R "$(dirname "$0")"/. "$target"/
chmod 755 "$target/bin/duaer-ai-desk-host-core${exe}"
