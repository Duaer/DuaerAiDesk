...
在 DuaerAiDesk 的**右侧工作面板**里浏览、查看并编辑当前项目的文件：完整目录树（图标随文件类型变化）、左右分栏（左侧文件列表可整块收起）、代码高亮编辑与保存、图片与音视频查看、CSV/JSON 结构化查看、在项目组注册了多个本地文件夹时切换当前浏览的那个 folder（按项目记住），以及新建、重命名/移动、按文件名搜索；宿主还可以让视图直接打开某个文件——包括它自己选定的项目外路径（会话临时目录 / 附件）。
...
- **跟随宿主外观**：亮/暗配色与界面语言（中文 / English）跟随 DuaerAiDesk 主体，编辑器与预览的高亮配色一并跟随。

## 安装

在 DuaerAiDesk 的 Plugins 页面：
