#!/bin/bash
# Open a trusted unsigned DuaerAiDesk installation on macOS.
#
...
readonly APP_BUNDLE_NAME="DuaerAiDesk.app"
readonly EXPECTED_BUNDLE_ID="net.aiuo.duaer-ai-desk"
...
if [[ -z "$app_path" ]]; then
  show_alert \
    "DuaerAiDesk is not installed" \
    "Drag DuaerAiDesk.app to Applications, then double-click this helper again."
  exit 1
fi
...
if [[ "$bundle_identifier" != "$EXPECTED_BUNDLE_ID" ]]; then
  show_alert \
    "This is not a DuaerAiDesk app" \
    "The helper only opens the official DuaerAiDesk bundle."
  exit 1
fi
...
if [[ "$quarantine_attributes" == *"com.apple.quarantine"* ]]; then
  if ! /usr/bin/xattr -r -d com.apple.quarantine "$app_path"; then
    show_alert \
      "DuaerAiDesk could not be opened" \
