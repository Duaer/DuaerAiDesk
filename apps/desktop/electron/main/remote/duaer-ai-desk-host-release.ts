/**
 * Release coordinates for the `duaer-ai-desk-host` bundle the desktop installs on a
 ...
 */
import { GITHUB_REPO } from "@duaer-ai-desk/shared";
...
/**
 * Release-matrix parity. `release.yml`'s `duaer-ai-desk-host-bundle` job publishes Linux
...
/** Directory name the tarball unpacks to, `bundle.mjs`'s `duaer-ai-desk-host-<version>-<platform>-<arch>`. */
export function piHostBundleDir(version: string, target: PiHostTarget): string {
  return `duaer-ai-desk-host-${version}-${targetKey(target)}`;
}
