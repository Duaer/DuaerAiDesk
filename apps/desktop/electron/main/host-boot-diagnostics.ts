 ...
 *
 * - `DB_SCHEMA_TOO_NEW`: a newer DuaerAiDesk already migrated the local data
...
export class DbSchemaTooNewError extends Error {
  ...
  constructor(schema: SchemaTooNew) {
    super(
      `Local data uses database schema ${schema.found}, but this build supports ${schema.supported}. Install the newer DuaerAiDesk that last opened this data.`,
    );
